...
// const ApplyLeaveModal = ({ currentEmployee, editingLeave, onClose, onSubmit }) => {
...
//               />
//               <div>
//                 <h3 className="font-medium text-slate-800 dark:text-white">{currentEmployee.name}</h3>
//                 <p className="text-sm text-slate-600 dark:text-slate-400">{currentEmployee.email}</p>
//               </div>
//             </div>
//           </div>
...
//           {/* ... (keep the duration, date selection, reason, and file upload sections) */}
...
//           {/* Leave Type - Radio buttons as in reference */}
//           <div>
//             <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
//               Leave Type *
//             </label>
...
import React, { useState, useEffect } from 'react';
import { X, Upload, Calendar, User, FileText, Clock, ChevronDown } from 'lucide-react';
          ...
          <div className="relative">
            ...
            >
              <div className="flex items-center gap-3">
                {formData.type ? (
                  <>
                    <span className="text-slate-800 dark:text-white font-medium">{formData.type}</span>
                    <span className="text-sm text-slate-600 dark:text-slate-400">
                      ({leaveTypes.find(lt => lt.name === formData.type)?.remaining} remaining)
                    </span>
                  </>
                ) : (
                  <span className="text-slate-500">Select leave type</span>
                )}
              </div>
            ...
            {isDropdownOpen && (
              <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl shadow-lg max-h-60 overflow-auto">
                {leaveTypes.map(type => (
                  <div
                    key={type.name}
                    onClick={() => selectLeaveType(type.name)}
