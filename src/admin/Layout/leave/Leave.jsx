...
const Leave = ({ currentEmployee }) => {
  ...
  // Current employee data (passed as prop)
  const currentEmployeeData = currentEmployee || {
    id: 1,
    name: 'Sarah Johnson',
  ...
  // Filter leave requests to only show current employee's requests
  const currentEmployeeLeaveRequests = leaveRequests.filter(
    req => req.employeeId === currentEmployeeData.id
  );

  const leaveStats = {
    totalRequests: currentEmployeeLeaveRequests.length,
    pending: currentEmployeeLeaveRequests.filter(req => req.status === 'pending').length,
    approved: currentEmployeeLeaveRequests.filter(req => req.status === 'approved').length,
    rejected: currentEmployeeLeaveRequests.filter(req => req.status === 'rejected').length
  };
  ...
  const getFilteredRequests = () => {
    let filtered = currentEmployeeLeaveRequests;
  ...
  const handleAddLeaveRequest = (newLeave) => {
    if (editingLeave) {
      ...
      // Add new leave - automatically set to current employee
      const leaveRequest = {
        id: Date.now(),
        ...newLeave,
        employeeId: currentEmployeeData.id,
        employeeName: currentEmployeeData.name,
        avatar: currentEmployeeData.avatar,
  ...
  return (
    <div className="space-y-6 py-6  max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">My Leave</h1>
      ...
      {/* Tab Navigation - Removed Calendar and Policies tabs
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/20 overflow-hidden">
            <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-800 dark:text-white">My Leave Requests</h2>
